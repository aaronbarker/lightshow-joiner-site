...
9. **Reset closures between shows** (on by default) injects a short lights-off tail — and matching silence — after a segment that actually moved windows, mirrors, liftgate, or charge port. The default pose is trunk/charge **closed** and windows/mirrors **open**. Tesla counts Open/Close/Dance per USB show (the joined file is one show): liftgate 6, each window 6, each mirror 20, charge port 3. Mid-playlist resets are skipped when a limit would be exceeded; the last show is still reset when budget allows. Idle/Stop do not count.
...
Because actuation limits apply to the **whole joined file**, the joiner only emits a reset for closures that were non-idle in that segment, always tries to reset the final segment, and skips mid-segment resets (with a warning) if adding them would exceed Tesla’s limits. If the source shows already used the full budget, no extra Close/Open is added.
